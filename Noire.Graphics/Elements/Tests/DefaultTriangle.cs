...
namespace Noire.Graphics.Elements.Tests
{
    public sealed class DefaultTriangle : DisplayObject
    {
        ...
        protected override void RenderInternal(RenderTarget target)
        {
            var outerVertex = new[]
            {
                new CustomVertex() { Position = new Vector4(400.0f, 62.5f, 0.5f, 1.0f), Color = new Color(255, 0, 0, 255) },
                new CustomVertex() { Position = new Vector4(650.0f, 500.0f, 0.5f, 1.0f), Color = new Color(0, 255, 0, 0) },
                new CustomVertex() { Position = new Vector4(150.0f, 500.0f, 0.5f, 1.0f), Color = new Color(0, 0, 255, 255) },
            };

            _vertexBuffer = new VertexBuffer(target.Device, outerVertex.Length * Utilities.SizeOf<CustomVertex>(), Usage.WriteOnly, VertexFormat.None, Pool.Managed);
            ...
            target.Device.SetStreamSource(0, _vertexBuffer, 0, Utilities.SizeOf<CustomVertex>());
